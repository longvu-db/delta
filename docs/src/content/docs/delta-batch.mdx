...
You can selectively overwrite only the data that matches an arbitrary expression. This feature is available with DataFrames in Delta Lake 1.1.0 and above and supported in SQL in Delta Lake 2.4.0 and above. To instead replace rows based on how they match the incoming DataFrame, see [Dynamically overwrite data with `replaceUsing` and `replaceOn`](#dynamically-overwrite-data-with-replaceusing-and-replaceon).
...
#### Choose a selective overwrite option

Delta Lake provides several options for selectively overwriting data. For most use cases, we recommend using `replaceUsing` or `replaceWhere`. Use `replaceOn` only when your use case requires complex or null-safe matching conditions. Avoid `partitionOverwriteMode`, which is deprecated.
...
| `partitionOverwriteMode` (deprecated) | Legacy dynamic partition overwrite. Not recommended. | Delta Lake 2.0 |
