...
case class DeltaInsertReplaceOnOrUsingCommand(
  ...
  private val withSchemaEvolution: Boolean = writeCmd.options.canMergeSchema
  ...
  private def materializeSource(
    ...
    //  cause we haven't implemented column pruning in materialization for
    //  INSERT REPLACE ON/USING yet. It is still constructed for clarity and future use.
    val replaceOnOrUsingCond: Expression = insertReplaceCriteria match {
      ...
      case InsertReplaceUsing(cols) => cols.map(col => EqualTo(UnresolvedAttribute(col),
        UnresolvedAttribute(Seq(replaceOnOrUsingTableAliasOpt.get, col)))).reduce(And)
      case other => throw new IllegalStateException(
        s"Unexpected InsertReplaceCriteria: $other")
    }

    {
      ...
      // Note: Dataset.ofRows must remain inside this QueryTagger.withTag block so that
      // MSTCheckRule sees the tag and skips re-analysis of the materialized version of the
      // source query plan.
...
object DeltaInsertReplaceOnOrUsingCommand {
   ...
   * REPLACE ON/USING implementation (see [[WriteIntoDeltaEdge]] and
   * [[DeltaAnalysis]]).
   */
   ...
   * implementation (see [[WriteIntoDeltaEdge]] and [[DeltaAnalysis]]).
   *
