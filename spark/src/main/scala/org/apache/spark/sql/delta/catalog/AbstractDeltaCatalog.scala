...
class AbstractDeltaCatalog extends DelegatingCatalogExtension
  ...
  private class StagedDeltaTableV2(
    ...
    override def commitStagedChanges(): Unit = recordFrameProfile(
      ...
      // Block replaceOn/replaceUsing with DFv2 create/replace/createOrReplace.
      // DFv1 saveAsTable sets __v1_save_as_table_overwrite in writeOptions
      // (Spark 4.1+). In Spark 4.0 this marker is absent, so DFv2 operations
      // with replaceOn/Using would be blocked; DFv1 saveAsTable with
      // replaceOn/Using is routed via DeltaAnalysis (insertInto) or
      // DeltaDataSource (save), not through this staged path.
      val ci = org.apache.spark.sql.catalyst.util
        .CaseInsensitiveMap(writeOptions)
      val isDfv1SaveAsTable = ci.contains(
        DeltaOptions.IS_DATAFRAME_WRITER_V1_SAVE_AS_TABLE_OVERWRITE)
      if (!isDfv1SaveAsTable &&
          (ci.contains(DeltaOptions.REPLACE_ON_OPTION) ||
          ci.contains(DeltaOptions.REPLACE_USING_OPTION))) {
        throw DeltaErrors
          .dfv2CreateReplaceIncompatibleReplaceOnOrUsing()
      }
      val conf = spark.sessionState.conf
