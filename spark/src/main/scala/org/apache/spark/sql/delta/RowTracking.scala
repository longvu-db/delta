...
object RowTracking {
  ...
  /**
   * Return a copy of tagsMap and if Row Tracking is enabled, also add the
   * [[DeltaCommitTag.PreservedRowTrackingTag.key]] to the copy.
   */
  private[delta] def addPreservedRowTrackingTagIfEnabled(
      protocol: Protocol,
      metadata: Metadata): Map[String, String] = {
    if (isEnabled(protocol, metadata)) {
      Map(DeltaCommitTag.PreservedRowTrackingTag.key -> "true")
    } else {
