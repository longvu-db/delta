...
trait DeltaInsertReplaceOnDFWriterTests
  ...
  for (materializationMode <- DeltaSQLConf.MergeMaterializeSource.list) {
    ...
    test(s"replaceOn with non-Delta source records materialization reason, $testParamMsg") {
      withSQLConf(
        ...
        withTempDir { dir =>
          ...
          withTable("source") {
            sql("CREATE TABLE source (col INT, row_origin STRING) USING csv")
            sql("INSERT INTO source VALUES (0, 'source'), (1, 'source'), " +
              "(2, 'source'), (3, 'source')")
  ...
  test("replaceOn with non-deterministic source records materialization reason") {
    withTempDir { dir =>
      val path = dir.getAbsolutePath
      Seq.tabulate(100)(i => (i + 1, "target")).toDF("col", "row_origin")
        .write.format("delta").save(path)

      withTable("source") {
        Seq.tabulate(100)(i => (i + 1, "source")).toDF("col", "row_origin")
        ...
        val events: Seq[UsageRecord] = Log4jUsageLogger.track {
          writeReplaceOnDF(
            sourceDF = spark.sql(
              "SELECT * FROM source WHERE col < trueRandom() * 100").as("s"),
            target = path,
            replaceOnCond = "t.col = s.col",
            targetAlias = Some("t"))
        }
  ...
  test("replaceOn disableSourceMaterializationNotAllowed errors when " +
      "materializeSource set to none") {
    def runWithNonDeltaSource(): Unit = {
      withTempDir { dir =>
        val path = dir.getAbsolutePath
        Seq((1, "target")).toDF("col", "row_origin")
          .write.format("delta").save(path)

        withTable("source") {
          sql("CREATE TABLE source (col INT, row_origin STRING) USING csv")
          sql("INSERT INTO source VALUES (1, 'source')")
