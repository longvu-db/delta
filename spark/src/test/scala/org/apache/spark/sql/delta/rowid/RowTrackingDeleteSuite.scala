...
import org.apache.spark.sql.delta.cdc.DeleteCDCSuite
...
trait RowTrackingDeleteTestDimension
   ...
   */
  def deleteAndValidateStableRowId(whereCondition: Option[String],
      checkPreservedRowTracking: Boolean = true): Unit = {
...
trait RowTrackingDeleteSuiteBase extends RowTrackingDeleteTestDimension {
  ...
  for (isPartitioned <- BOOLEAN_DOMAIN) {
    ...
    val whereClause = s"id > $initialNumRows" // 0 row match
    // In Delta, we use Serializable, this means that we don't commit when we don't modify
    // anything, this makes the checkRowTrackingMarkedAsPreservedForCommit fails because
    // it relies on the [[CommitInfo]] action committed.
    test(s"DELETE preserves Row IDs, isPartitioned=$isPartitioned," +
      s"whereClause=`$whereClause`") {
      withRowIdTestTable(isPartitioned) {
        deleteAndValidateStableRowId(Some(whereClause), checkPreservedRowTracking = false)
      }
    }
  }

  test("Preserving Row Tracking - Subqueries are not supported in the Delete") {
...
trait DeleteWithRowTrackingTests extends DeleteSQLSuite with RowTrackingTestUtils {
  ...
  override def excluded: Seq[String] = super.excluded ++
    Seq(
      // TODO: DELETE on views can't find metadata column.
      ...
      "test delete on temp view - view with too many internal aliases with write amplification " +
        "reduction - SQL TempView",
      "test delete on temp view - view with too many internal aliases with write amplification " +
        "reduction - Dataset TempView",
      "test delete on temp view - nontrivial projection with write amplification " +
        "reduction - SQL TempView",
      "test delete on temp view - nontrivial projection with write amplification " +
        "reduction - Dataset TempView",
      "test delete on temp view - nontrivial projection - SQL TempView",
      "test delete on temp view - superset cols - Dataset TempView",
      "delete from a SQL temp view",
...
// DELETE + row tracking
class DeleteWithRowTrackingSuite extends DeleteSQLSuite with DeleteWithRowTrackingTests {
  override protected def sparkConf: SparkConf = super.sparkConf
    .set(DeltaConfigs.ENABLE_DELETION_VECTORS_CREATION.defaultTablePropertyKey, "false")
}

// DELETE + CDC + row tracking
class DeleteWithRowTrackingCDCSuite extends DeleteCDCSuite with DeleteWithRowTrackingTests {
  override protected def sparkConf: SparkConf = super.sparkConf
    .set(DeltaConfigs.ENABLE_DELETION_VECTORS_CREATION.defaultTablePropertyKey, "false")
}
...
// are missing.
trait DeleteWithRowTrackingTableFeatureTests extends DeleteSQLSuite with RowTrackingTestUtils {
  override protected def sparkConf: SparkConf = super.sparkConf
    .set(DeltaConfigs.ROW_TRACKING_ENABLED.defaultTablePropertyKey, "false")
    .set(defaultRowTrackingFeatureProperty, "supported")
}

// DELETE + row tracking table feature
class DeleteWithRowTrackingTableFeatureSuite
  extends DeleteSQLSuite
  with DeleteWithRowTrackingTableFeatureTests {
  override protected def sparkConf: SparkConf = super.sparkConf
    .set(DeltaConfigs.ENABLE_DELETION_VECTORS_CREATION.defaultTablePropertyKey, "false")
}

// DELETE + CDC + row tracking table feature
class DeleteWithRowTrackingTableFeatureCDCSuite
  extends DeleteCDCSuite
  with DeleteWithRowTrackingTableFeatureTests {
  override protected def sparkConf: SparkConf = super.sparkConf
    .set(DeltaConfigs.ENABLE_DELETION_VECTORS_CREATION.defaultTablePropertyKey, "false")
}
