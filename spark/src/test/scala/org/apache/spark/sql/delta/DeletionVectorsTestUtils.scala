...
trait DeletionVectorsTestUtils extends QueryTest with SharedSparkSession with DeltaSQLTestUtils {
  ...
  /**
   * Add deletion vectors to all files of a table that don't already have one, by duplicating the
   * Parquet files and adding DVs that shard the rows between the two files.
   * When `toAllFiles` is false, DV will be added to only one random file.
   */
  def addDeletionVectorsToTableLog(log: DeltaLog, toAllFiles: Boolean = true): Unit = {
    ...
    for (origFile <- filesToBeAdded if origFile.deletionVector == null) {
      ...
      // Copy data file to a new file.
      val oldRelPath = origFile.toPath
      val newFileName = s"test-copy-of-${oldRelPath.getName}"
      ...
      // Delete half rows from one file and half from other.
